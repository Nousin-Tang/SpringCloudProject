...
import com.nousin.springcloud.server.storage.framework.common.util.CookieUtil;
...
public class Page<T> implements Serializable{
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		if (StringUtils.isNumeric(no)){
			...
			CookieUtil.setCookie(response, "pageNo", "1");
			this.setPageNo(Integer.parseInt("1"));
		}
		...
		if (StringUtils.isNumeric(size)){
			...
			size = CookieUtil.getCookie(request, "pageSize");
			if (StringUtils.isNumeric(size)){
				this.setPageSize(Integer.parseInt(size));
			}
        ...
        if (StringUtils.isNotBlank(funcName)){
            CookieUtil.setCookie(response, "funcName", funcName);
            this.setFuncName(funcName);
        }else if (request.getParameter("repage")!=null){
            funcName = CookieUtil.getCookie(request, "funcName");
            if (StringUtils.isNotBlank(funcName)){
                this.setFuncName(funcName);
            }
        }
		...
		if (StringUtils.isNotBlank(orderBy)){
			...
			if(request.getAttribute("orderBy") != null){
				String orderBy2 = String.valueOf(request.getAttribute("orderBy"));
				if(StringUtils.isNotBlank(orderBy2)){
					this.setOrderBy(orderBy2);
				}
			}
		}
	}
